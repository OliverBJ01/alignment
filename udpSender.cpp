...
int zsockfd,  slen=sizeof(servaddr);
...
void sendMssg(uint x, uint y, bool laserOn)  {
	...
	if (sendto (zsockfd,  & mssg, sizeof(struct opencvMssg) , 0 , (struct sockaddr *) &servaddr, slen)==-1)      {
...
int initQtSocket(void)   {
	...
	// create socket
	if ((zsockfd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1) {
